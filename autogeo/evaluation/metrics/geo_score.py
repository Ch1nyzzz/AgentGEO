...
def extract_citations_new(text: str) -> list:
    ...
    # Preprocessing: convert ".[number]" format to "[number]." to prevent NLTK from splitting citations into separate sentences
    # Example: "...damage.[6]" -> "...damage[6]."
    # Supports consecutive citations: "...damage.[6][7]" -> "...damage[6][7]."
...
def impression_wordpos_count_simple(sentences: list, n: int = 5, normalize: bool = True) -> list:
    ...
    for i, sent in enumerate(sentences):
        for cit in sent[2]:
            ...
            score /= len(sent[2])
            try:
                if cit >= 1:  # Citation index is 1-based, convert to 0-based
                    scores[cit - 1] += score
            except IndexError:
                pass
