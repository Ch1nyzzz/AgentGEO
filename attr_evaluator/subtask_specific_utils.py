...
def extract_json_from_response(response: str, expect_array: bool = True) -> str:
    ...
    if end_idx == -1:
        raise ValueError(f"Unmatched {start_char}{end_char} in response")
...
sent_transformer_tokenizer = 1 #AutoTokenizer.from_pretrained(sent_transformer_model_name)
sent_transformer_model = 1 #AutoModel.from_pretrained(sent_transformer_model_name)
nlp = 1


def create_modified_dataset(outdir, incomplete_cite_list, alignments_dict, mode_idx=0, data_num=50):
    attack_plan = {}
   
    def apply_attack(example, idx): # 加一个如果没出现就删除
        example["origin_id"] = idx
        if idx in attack_plan:
            idx_to_attack = attack_plan[idx]
            if 0 <= idx_to_attack < len(example['sources']):
                
                new_documents = example['sources']
                if mode_idx == 20000:
                    new_documents = [new_documents[idx_to_attack]]
                else:
                    # 对选中的文档应用攻击
                    new_documents[idx_to_attack] = attack_method_by_index(new_documents[idx_to_attack], mode_idx)
                
                # 将修改后的列表_替换_掉 example 中的旧列表
                example['sources'] = new_documents
        return example
    # 2. 定义 filter 函数（只删除，不修改）
    def filter_function(example, idx):
        return idx in attack_plan
    alignments_dict = alignments_dict.map(apply_attack, with_indices=True)  
    alignments_dict = alignments_dict.filter(filter_function, with_indices=True)
...
def get_indir_paths(args):
    ...
    align_path = Path(indir_alignments)
    prompt_path = Path(indir_prompt)
    if not align_path.is_absolute():
        align_path = (base_dir / align_path).resolve()
    if not prompt_path.is_absolute():
        prompt_path = (base_dir / prompt_path).resolve()
    return str(prompt_path), str(align_path)
...
def get_data(args):
    ...
    # load dataset and (insert trigger)
    if("pipeline_format_results" in indir_alignments):
        with open(indir_alignments, 'r') as f1:
            file_content = f1.read().strip()
            if file_content:
                try:
                    # 尝试解析为单个JSON对象
                    single_data = json.loads(file_content)
                    alignments_dict = [single_data]  # 包装成列表
                except json.JSONDecodeError:
                    # 如果解析失败，尝试按行解析
                    lines = file_content.splitlines()
                    alignments_dict = [json.loads(line.strip()) for line in lines if line.strip()]
    else:
        attack_mode = getattr(args, 'attack_mode', None)
        outdir = getattr(args, "outdir", "./")
        if attack_mode=="insert":
            alignments_dict = load_dataset('GEO-optim/geo-bench', 'test')['test']
            with open(indir_alignments, 'r') as f1:
                trigger_data = json.loads(f1.read())
            alignments_dict = create_modified_dataset(outdir, trigger_data['incomplete_cite_list'], alignments_dict, args.idx_to_attack, args.data_num)
            ...
            alignments_dict = load_dataset('GEO-optim/geo-bench', 'test')['test']
            choose_idx = getattr(args, 'choose_idx', None)

            if choose_idx:
                if isinstance(choose_idx, str):
                    try:
                        choose_list = [int(x.strip()) for x in choose_idx.split(',') if x.strip()]
                    except Exception:
                        raise ValueError(f"cannot parse choose_idx={choose_idx}")
                elif isinstance(choose_idx, list):
                    choose_list = [int(x) for x in choose_idx]
                else:
                    raise ValueError("choose_idx must be a comma-separated string or a list of ints")

                alignments_dict = alignments_dict.select(choose_list) 
...
def construct_non_demo_part_new(instance, idx, merge_cross_sents_highlights, specific_prompt_details, prompt_dict, no_highlights, cut_surplus: bool = False, prct_surplus: float = 0.25):
        ...
        topic_name = idx
        highlighted_texts = get_highlighted_doc_new(docs={idxx:elem['raw_text'] for idxx, elem in enumerate(instance['sources'][idx]) if elem['raw_text']}, 
                                                ...
                                                doc_sents={idxx:elem['raw_text'] for idxx, elem in enumerate(instance['sources'][idx]) if elem['raw_text']} )
        ...
        if cut_surplus:
            if no_highlights: # tasks without given highlights - remove prct_surplus% of the top sentences (unless it's less than 5 sentences, and then keep the first 5 sentences - to avoid cases when short documents completely disappear)
                # docs={idxx:elem['raw_text'] for idxx, elem in enumerate(instance['sources'][idx]) if elem['raw_text']}
                highlighted_texts = {idxx:"".join(elem['raw_text'][:max(int(len(elem['raw_text'])*(1-prct_surplus)), 5)]) for idxx, elem in enumerate(instance['sources'][idx]) if elem['raw_text']}
        ...
        if "query" in instance.features: # for LFQA
            eval_item["question"] = instance["query"][idx]
        if "origin_id" in instance.features:
            eval_item["origin_id"] = instance["origin_id"][idx]

        curr_prompt = make_demo_new(
        ...
        return curr_prompt, topic_name, docs_order
...
def construct_non_demo_part(instance, merge_cross_sents_highlights, specific_prompt_details, prompt_dict, no_highlights, cut_surplus: bool = False, prct_surplus: float = 0.25):
        ...
        highlight_start_tkn = "{HS}"
        highlight_end_tkn="{HE}"
        topic_name = instance['id']
        highlighted_texts = get_highlighted_doc(docs={idxx:elem['raw_text'] for idxx, elem in enumerate(instance['document']) if elem},
        ...
        if cut_surplus:
            if no_highlights: # tasks without given highlights - remove prct_surplus% of the top sentences (unless it's less than 5 sentences, and then keep the first 5 sentences - to avoid cases when short documents completely disappear)
                highlighted_texts = {idxx:"".join(elem['raw_text'][:max(int(len(elem['raw_text'])*(1-prct_surplus)), 5)]) for idxx, elem in enumerate(instance['document']) if elem}
            else: # tasks with given highligts (e.g., clustering, FiC) - remove everything after last highlight
                highlighted_texts = {doc_name:rmv_txt_after_last_highlight(doc_text, highlight_end_tkn) for doc_name,doc_text in highlighted_texts.items()}
                highlighted_texts = {key:value for key,value in highlighted_texts.items() if value} # remove empty texts
        ...
        if "question" in instance.keys(): # for LFQA
            eval_item["question"] = instance["question"]
        if "origin_id" in instance.keys():
            eval_item["origin_id"] = instance["origin_id"]
...
def construct_prompts(prompt_dict : Dict, alignments_dict : List[Dict], n_demos : int, debugging : bool, merge_cross_sents_highlights : bool, specific_prompt_details : Dict, tkn_counter: Dict, no_highlights : bool = False, cut_surplus : bool = False, prct_surplus: float = None):
    ...
    for instance in alignments_dict:
        ...
        if "question" in instance.keys(): # for LFQA
            additional_data[topic_name].update({"question":instance["question"]})
        if "origin_id" in instance.keys():
            additional_data[topic_name].update({"origin_id":instance["origin_id"]})
    return used_demos, final_prompts, additional_data


def construct_prompts_new(prompt_dict : Dict, alignments_dict : List[Dict], n_demos : int, debugging : bool, merge_cross_sents_highlights : bool, specific_prompt_details : Dict, tkn_counter: Dict, no_highlights : bool = True, cut_surplus : bool = False, prct_surplus: float = None):
    # Generate the demonstration part
    head_prompt, head_prompt_shorter = "", "" # the shorter is for cases when the instance with the demos is too long
    train_ids = np.random.choice(len(prompt_dict["demos"]), n_demos, replace=False) if not debugging else [0,2]
    used_demos = []
    for train_id in train_ids:
        ...
        curr_prompt_demo= make_demo_new(
            item=train_item, prompt=specific_prompt_details["demo_prompt"], doc_prompt=prompt_dict["doc_prompt"], 
            instruction=specific_prompt_details["instruction_prompt"], answer_related_prompts=specific_prompt_details["answer_related_prompts"],
            highlight_start_tkn=prompt_dict["highlight_start_tkn"], highlight_end_tkn=prompt_dict["highlight_end_tkn"],
            content_selection=no_highlights
        ...
        curr_prompt_demo_shorter = make_demo_new(
            item=train_item_shorter, prompt=specific_prompt_details["demo_prompt"], doc_prompt=prompt_dict["doc_prompt"],
            instruction=specific_prompt_details["instruction_prompt"], answer_related_prompts=specific_prompt_details["answer_related_prompts"],
            highlight_start_tkn=prompt_dict["highlight_start_tkn"], highlight_end_tkn=prompt_dict["highlight_end_tkn"],
            content_selection=no_highlights
    ...
    final_prompts, additional_data = {}, {}
    for idx in range(alignments_dict.num_rows):
        # instance = alignments_dict['source'][idx]
        curr_prompt, topic_name, docs_order = construct_non_demo_part_new(alignments_dict, idx, merge_cross_sents_highlights, specific_prompt_details, prompt_dict, no_highlights)        
        

        if cut_surplus: # cut text
            curr_prompt, topic_name, docs_order_shorter = construct_non_demo_part_new(alignments_dict, idx, merge_cross_sents_highlights, specific_prompt_details, prompt_dict, no_highlights, cut_surplus=True, prct_surplus=prct_surplus)
            final_prompts[topic_name] = head_prompt_shorter + curr_prompt
        elif tkn_counter["tkn_counter"].token_count(head_prompt + curr_prompt)>=tkn_counter["tkn_max_limit"]: # the current prompt is too long - need to "cut" the surplus texts
            # check several such cut-off thr (until the first/smallest one catches) - rage from 0.05 and 0.5
            prct_surplus_lst = [0.5, 0.6, 0.7]
            for curr_prct_surplus in prct_surplus_lst:
                curr_prompt, topic_name, docs_order_shorter = construct_non_demo_part_new(alignments_dict, idx, merge_cross_sents_highlights, specific_prompt_details, prompt_dict, no_highlights, cut_surplus=True, prct_surplus=curr_prct_surplus)
                if tkn_counter["tkn_counter"].token_count(head_prompt_shorter + curr_prompt)<tkn_counter["tkn_max_limit"]:
                    break
            final_prompts[topic_name] = head_prompt_shorter + curr_prompt
        else:
        ...
        if "query" in alignments_dict.features: # for LFQA
            additional_data[topic_name].update({"question":alignments_dict["query"][idx]})
        if "origin_id" in alignments_dict.features:
            additional_data[topic_name].update({"origin_id":alignments_dict["origin_id"][idx]})
    return used_demos, final_prompts, additional_data
...
def parse_clustering_response(response, prompt):
    ...
    # 使用健壮的 JSON 提取方法，处理 LLM 响应中的额外文本
    json_str = extract_json_from_response(response, expect_array=True)
    clusters = json.loads(json_str)
    # 这个东西就是一个list里有很多json 0: {'cluster': [1, 8, 9, 10, 11]}
...
def get_set_of_highlights_in_context_content_selection(doc_name, doc_text, highlights, nlp=None, doc_sents=None, *args, **kwargs):
    ...
    for h in highlights:
        ...
        highlights_in_context_list.append({
            "documentFile": doc_name,
            "scuSentCharIdx": None,
            "scuSentence": None,
            "docSentCharIdx": str(h_idx_limits[0]),
            "docSentText": doc_text,
...
def convert_content_selection_results_to_pipeline_format(results, alignments_dict, *args, **kwargs):
    ...
    # 确保alignments_dict是字典格式
    if isinstance(alignments_dict, list):
        # 如果是列表，转换为字典
        alignments_dict = {elem['id']: elem for elem in alignments_dict}
    
    for key,value in results.items():
        origin_id_val = results[key].get('origin_id')
        curr_original_inst = {'id': key, 
                              'document': alignments_dict[key]['document'],
                            'question': alignments_dict[key]['question'] if alignments_dict[key]['question'] else None,
                            'origin_id': alignments_dict[key].get('id', None)
                              }
        
        curr_documents = curr_original_inst["document"]
        ...
        highlights_in_context = [get_set_of_highlights_in_context_content_selection(doc_name=doc['doc_name'], 
                                                                  doc_text=[elem['raw_text'] for idxx, elem in enumerate(curr_documents) if idxx==doc['doc_name']][0],
                                                                  highlights=value["final_output"][f"Document [{str(i+1)}]"], 
                                                                  nlp=nlp, 
                                                                  doc_sents=None) for i,doc in enumerate(value['non_highlighted_docs']) if f"Document [{str(i+1)}]" in value["final_output"].keys()]
...
def get_set_of_highlights_in_context_clustering(curr_instance, nlp, doc_sents, *args, **kwargs):
    ...
    for doc_i,doc_highlights in enumerate(curr_instance['highlights']):
        ...
        curr_doc_text = next(iter([elem['raw_text'] for idxx, elem in enumerate(doc_sents) if idxx==curr_doc_name]), None)
        curr_doc_sents = None #next(iter([elem['documentText'] for idxx, elem in enumerate(doc_sents) if idxx==curr_doc_name]), None)
...
def convert_clustering_results_to_pipeline_format(results, alignments_dict, *args, **kwargs):
    ...
    # 确保alignments_dict是列表格式
    if isinstance(alignments_dict, dict):
        # 如果是字典，转换回列表
        alignments_dict = list(alignments_dict.values())
    ...
    for key,value in results.items():
        # 查找对应的实例
        if key in alignments_dict_dict:
            curr_pipeline_style_data = alignments_dict_dict[key]
        else:
            # 如果找不到，创建一个新的实例
        ...
        curr_documents = curr_pipeline_style_data["document"]
        ...
        # 从curr_pipeline_style_data中获取question和origin_id，而不是从alignments_dict[key]中获取
        curr_pipeline_style_data.update({"set_of_highlights_in_context":highlights_in_context,
                                         'question': curr_pipeline_style_data.get('question'),
                                         'origin_id': curr_pipeline_style_data.get('origin_id')
                                       })
