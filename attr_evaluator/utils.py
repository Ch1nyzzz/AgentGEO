...
class TokenCounter:
    def __init__(self, model_name: str):
        ...
        # elif 'claude' in self.model_name: # <-- 2. Add this block
        #     # This assumes you have the ANTHROPIC_API_KEY environment variable set
        #     self.client = anthropic.Anthropic()
        # elif 'qwen' in self.model_name:  # <-- 2. 添加 'qwen' 的处理逻辑
        #     # 使用 dashscope SDK 提供的官方 tokenizer
        #     # 这将在本地加载分词器，而不是 API 调用
        #     try:
        #         self.tokenizer = dashscope.get_tokenizer(self.model_name)
        #     except Exception as e:
        #         print(f"警告: 无法加载 {self.model_name} 的分词器。 "
        #               f"将回退使用 'qwen-turbo' 的分词器。 错误: {e}")
        #         # 如果模型名称不被 'get_tokenizer' 直接支持（例如变体），
        #         # 回退到一个已知的基础模型
        #         self.tokenizer = dashscope.get_tokenizer("qwen-turbo")
        # else:
    ...
    def token_count(self, prompt):
        ...
        #     usage = self.client.count_tokens(prompt, model=self.model_name)
...
def get_max_memory():
    ...
    max_memory_dict['cpu'] = f"{min(int(available_memory_gb), 100)}GiB"
...
def remove_spaces_and_punctuation(text):
    cleaned = re.sub(r'[' + re.escape(string.punctuation) + r'\s]+', '', text)
...
def get_highlighted_doc_new(docs, highlight_start_tkn, highlight_end_tkn, merge_cross_sents_highlights: bool = False, doc_sents: List = None):
    highlighted_docs = {}
    for doc_name, doc_text in docs.items():
        # add highlights
        highlighted_docs[doc_name] = add_highlights(doc_text, None, highlight_start_tkn, highlight_end_tkn)
...
def claude_call(prompt, model_name, output_max_length: int = 2048, temperature: int = 0):
...
def qwen_call(prompt, model_name, output_max_length: int = 2048, temperature: int = 0):
    messages = [
                {"role": "system", "content": "You are a helpful assistant."},
                {"role": "user", "content": prompt}
            ]
    response = dashscope.Generation.call(
        model=model_name,
        messages=messages,
        result_format='message',  # 设置为 'message' 来获取与 OpenAI 兼容的输出
        max_tokens=output_max_length,
        temperature=temperature
    )
...
def openai_call(prompt, model_name, output_max_length: int = 4096, temperature: int = 0):
...
def model_call_wrapper(prompt: str, model_name: str, parse_response_fn, output_max_length : int = 4096, num_retries: int = 5, temperature: int = 0):
    if "gpt" in model_name:
        call_func = openai_call
    elif "gemini" in model_name:
        call_func = gemini_call
    elif "qwen" in model_name:
        call_func = qwen_call
    elif "claude" in model_name:
        call_func = claude_call  # 假设您有一个 claude_call 函数
    else:
        # 总是处理未知情况
...
def make_demo_new(item, prompt, doc_prompt=None, instruction=None, answer_related_prompts=None, highlight_start_tkn=None, highlight_end_tkn=None, test=False, content_selection=False):
    # For demo prompt
    # - {INST}: the instruction
    # - {Q}: the questions (relevant for LFQA)
    # - {D}: the documents
