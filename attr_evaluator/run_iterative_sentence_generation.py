...
def iterative_sent_gen_prompting(alignments_dict, prompt_dict, used_demos, model_name, num_retries, debugging, n_demos, num_demo_changes, temperature, merge_cross_sents_highlights, tkn_counter, cut_surplus: bool = False, always_with_question: bool = False, no_prefix: bool = False):
    ...
    alignments_dict = [elem for elem in alignments_dict if not elem['id'] in ['test59', 'test62', 'test63', 'test67', 'test91']]
    if debugging:
    ...
    for instance in tqdm(alignments_dict):
        ...
        final_data_instances[topic_name] = {"non_highlighted_docs_full": {idxx:elem["raw_text"] for idxx, elem in enumerate(instance["document"])}}
        ...
        for i,scuSentCharIdx in enumerate(curr_scuSentCharIdxs):
            ...
            for demo_i in range(num_demo_changes+1):
                ...
                curr_step_prompt, curr_additional_data = construct_curr_prompt(prompt_dict=prompt_dict,
                                                                               ...
                                                                               curr_docs={idxx:elem["raw_text"] for idxx, elem in enumerate(instance["document"])},
                                                                               ...
                                                                               instance_question=instance['question'] if 'question' in instance.keys() else None,
                                                                               no_prefix = no_prefix)
...
def convert_iterative_sent_gen_to_pipeline_format(results, alignments_dict, *args, **kwargs):
    ...
    for key,value in results.items():
        curr_original_inst = deepcopy([elem for elem in alignments_dict if elem["id"]==key][0])
...
def main(args):
    ...
    for instance_name, resp in responses.items():
        # final_results[instance_name]['gold_summary'] = [elem['response'] for elem in alignments_dict if elem['id']==instance_name][0]
        final_results[instance_name] = resp
